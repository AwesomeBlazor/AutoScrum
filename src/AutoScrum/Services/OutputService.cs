...
{
    private readonly DailyScrumService _dailyScrum;

    private readonly DateService _dateService = new();

    public OutputService(DailyScrumService dailyScrum)
    {
        _dailyScrum = dailyScrum;
    }

    public MarkupString MarkupOutput { get; set; }

    public event EventHandler Updated;

    public void Update()
    {
        var markdown = GenerateReport(_dailyScrum.Users.Where(x => x.Included).ToList());
        var html = Markdig.Markdown.ToHtml(markdown);
        MarkupOutput = (MarkupString)html;
        
        Updated?.Invoke(this, EventArgs.Empty);
    }

    private string GenerateReport(List<User> users, bool isMarkdown = true)
    {
        var yesterday = _dateService.GetPreviousWorkDay(_dailyScrum.TodayDay);
        return isMarkdown
            ? GenerateMarkdownReport(_dailyScrum.TodayDay, yesterday, _dailyScrum.Today, _dailyScrum.Yesterday, users)
            : GeneratePlainTextReport(_dailyScrum.TodayDay, yesterday, _dailyScrum.Today, _dailyScrum.Yesterday);
    }

    private static string GenerateMarkdownReport(DateOnly todayDay, DateOnly previousDay, List<WorkItem> today, List<WorkItem> yesterday, List<User> users)
    {
    ...
    private static string GeneratePlainTextReport(DateOnly todayDay, DateOnly previousDay, List<WorkItem> today, List<WorkItem> yesterday)
    {
        // All days except for Monday will have "Yesterday", otherwise "Friday".
        // NOTE: MVP doesn't support flexible dates like not working on project for X day and then coming back. (or work on weekends)
        var previousDayName = "Yesterday";
